...
def prdc5k(opts):
    opts.dataset_kwargs.update(max_size=None, xflip=False)
    dict_ = prdc.compute_prdc(opts, max_real=5000, num_gen=5000,nearest_k=5)
    return dict(precision=dict_['precision'], recall=dict_['recall'], density=dict_['density'], coverage=dict_['coverage'])
    
@register_metric
def prdc50k(opts):
    opts.dataset_kwargs.update(max_size=None, xflip=False)
    dict_ = prdc.compute_prdc(opts, max_real=50000, num_gen=50000,nearest_k=5)
    return dict(precision=dict_['precision'], recall=dict_['recall'], density=dict_['density'], coverage=dict_['coverage'])
